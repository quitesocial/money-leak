...
const mockAddCategory = jest.fn<(name: string) => Promise<void>>();
...
beforeEach(() => {
  ...
  mockAddCategory.mockImplementation(async (name) => {
...
describe('AddTransactionScreen', () => {
  ...
  it('shows a created category and can use it for transaction creation', async () => {
    const renderer = await renderAddTransactionScreen();
    ...
    expect(mockAddCategory).toHaveBeenCalledWith('Travel');
    expect(mockAddTransaction).toHaveBeenCalledWith(
      expect.objectContaining({
        amount: 15,
        category: 'travel',
